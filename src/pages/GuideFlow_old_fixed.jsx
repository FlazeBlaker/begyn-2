...
const Step4ToN_Dynamic = ({ step, stepNumber, totalSteps, data, updateData, next, finish }) => {
    ...
    const handleNext = () => {
        const answerToSave = step.type === 'radio' || step.type === 'select' ? radioAnswer : currentAnswer;
    ...
    const isNextDisabled = step.required && ((step.type === 'text' && !currentAnswer) || ((step.type === 'radio' || step.type === 'select') && !radioAnswer));
...
export default function GuideFlow({ setOnboardedStatus }) {
    ...
    const finishGuide = async () => {
        ...
        try {
            const finalGuideResponseString = await generateContent({
                type: "finalGuide",
                payload: {
