...
export default function YourGuidePage({ userInfo, setOnboardedStatus }) {
    ...
    const handleSubNodeComplete = async (stepId, subNodeIndex, totalSubNodes) => {
        if (!uid) return;

        try {
            const currentStepProgress = roadmapProgress[stepId] || {};
            const currentSubNodes = currentStepProgress.subNodes || [];

            // Create new array with updated status
            const newSubNodes = [...currentSubNodes];
            // Ensure array is long enough if it was empty/partial
            while (newSubNodes.length <= subNodeIndex) {
                newSubNodes.push(false);
            }
            newSubNodes[subNodeIndex] = true;

            const updates = {
                [`roadmapProgress.${stepId}.subNodes`]: newSubNodes
            };

            // Check if all subnodes are now complete
            // We need the total count of subnodes for this step to know if we are done
            if (totalSubNodes > 0) {
                // Count true values in newSubNodes, treating undefined/missing as false
                // Actually, we need to check if *every* index from 0 to totalSubNodes-1 is true.
                // But newSubNodes might be sparse if we only clicked the last one?
                // No, we should assume we want to check all.

                // Let's just update the subnodes for now. Auto-completing the parent step 
                // requires robust checking against the definition.
                // Roadmap.jsx does: const allSubsComplete = updatedSubNodes.every((s) => s.completed);
                // But it works on the full node object.
                // Here we just have the boolean array.

                // Simple check: if we have enough true values?
                // Let's leave auto-complete for the user to click "Mark Done" on the step for now, 
                // to avoid accidental completion if logic is slightly off.
            }

            await updateDoc(doc(db, "brands", uid), updates);
        } catch (error) {
            console.error("Error updating subnode progress:", error);
        }
    };
    ...
    const mobileSteps = dynamicRoadmapSteps.map(step => {
        ...
        return {
            ...
            subNodes: (step.subNodes || []).map((sub, i) => ({
                ...sub,
                completed: stepProgress.subNodes ? stepProgress.subNodes[i] : false
            }))
        };
    });
            ...
            < div className="mobile-header" style={{
                ...
                <div style={{ flex: '1 1 auto', minWidth: '200px' }}>
                    ...
                    <p style={{
                        ...
                        Your <span style={{ color: '#e2e8f0', fontWeight: '600' }}>Begyn</span> command center for content domination.
                    </p>
                </div>
                {!isGenerating && (
                    ...
                    loadingRoadmap ? <LoadingCard height="400px" title /> : (
                        isMobile ? (
                            <MobileRoadmap
                                ...
                                onSubNodeComplete={handleSubNodeComplete}
                            />
                        ) : (
                            <>
